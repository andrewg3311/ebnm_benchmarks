...
benchmark_ebnm_normal = function(ebnm_fn = ebnm_normal, B = 1000, n = 1000, mu = 0, a = .25, pi0 = 0, s_alpha = 2, s_beta = 20, g_start = NULL, bench_times = 5, bench_name = "", seed = 1138) {
  set.seed(seed)
...
compare_res = function(res_list, true_par1, true_par2) {
  ...
  # Avg Time
  boxplot(avg_time ~ name, data = res_df, ylab = "Average Runtime (milliseconds)", main = "Comparison of Average Runtimes")
  points(jitter(as.numeric(as.factor(res_df$name))), res_df$avg_time, col = rep(cols, times = sapply(res_list, nrow)))
  
  # MSE
  boxplot(MSE ~ name, data = res_df, ylab = "MSE", main = "Comparison of MSEs")
  points(jitter(as.numeric(as.factor(res_df$name))), res_df$MSE, col = rep(cols, times = sapply(res_list, nrow)))
  
  # log-lik
  boxplot(ll ~ name, data = res_df, ylab = "Log-Likelihood", main = "Comparison of Log-Likelihoods")
  points(jitter(as.numeric(as.factor(res_df$name))), res_df$ll, col = rep(cols, times = sapply(res_list, nrow)))
  ...
  plot_ind = (abs(d - true_par2) / true_par2 <= 1)
  ...
  if (sum(!plot_ind) > 1) { # if relative error > 100%
...
res_list = list(benchmark_ebnm_normal(B=100, bench_name = "Default Start", mu = mu, a = a), 
                benchmark_ebnm_normal(B=100, g_start = list(mu = mu, a = a), bench_name = "Truth Start", mu = mu, a = a),
                benchmark_ebnm_normal(B=100, g_start = list(mu = 1, a = 1), bench_name = "(1, 1) Start", mu = mu, a = a), 
                benchmark_ebnm_normal(B=100, g_start = list(mu = -1, a = 1), bench_name = "(-1, 1) Start", mu = mu, a = a), 
                benchmark_ebnm_normal(B=100, g_start = list(mu = 100, a = .25), bench_name = "(100, .25) Start", mu = mu, a = a), 
                benchmark_ebnm_normal(B=100, g_start = list(mu = 0, a = 100), bench_name = "(0, 100) Start", mu = mu, a = a))

compare_res(res_list, mu, a)
```
